                ...
                double X_val[num_val][num_inputs], double Y_val[num_val][num_outputs])
{
    ...
    for (int ep = 1; ep <= epochs; ep++) {
        ...
        if (1) {
            ...
            double ***a_temp = malloc((num_hidden_layers + 1) * sizeof(double **));
            for (int layer = 0; layer <= num_hidden_layers; layer++) {
                int num_neurons_current_layer = (layer == num_hidden_layers) ? num_outputs : num_neurons[layer];
                a_temp[layer] = malloc(num_neurons_current_layer * sizeof(double *));
                for (int neuron = 0; neuron < num_neurons_current_layer; neuron++) {
                    a_temp[layer][neuron] = malloc(num_train * sizeof(double));
                }
            }
            ...
            // print a_temp[num_hidden_layers]
            printf("a_temp[%d]:\n", num_hidden_layers);
            for (int i = 0; i < num_outputs; i++) {
                for (int j = 0; j < num_train; j++) {
                    printf("%lf ", a_temp[num_hidden_layers][i][j]);
                }
                printf("\n");
            }
            ...
            double **output_neurons = malloc(num_train * sizeof(double *));
            for (int i = 0; i < num_train; i++) {
                output_neurons[i] = malloc(num_outputs * sizeof(double));
            }
            // Transpose a_temp[num_hidden_layers] to output_neurons
            for (int i = 0; i < num_outputs; i++) {
                for (int j = 0; j < num_train; j++) {
                    output_neurons[j][i] = a_temp[num_hidden_layers][i][j];
                }
            }

            // print output neurons
            printf("output_neurons:\n");
            for (int i = 0; i < num_train; i++) {
                for (int j = 0; j < num_outputs; j++) {
                    printf("%lf ", output_neurons[i][j]);
                }
                printf("\n");
            }
            printf("\n");
            ...
            for (int i = 0; i < num_train; i++) {
                for (int j = 0; j < num_outputs; j++) {
                    double diff = output_neurons[i][j] - Y_train[i][j];
                    printf("%d, %d | %f - %f = %f\t%f\n", i, j, output_neurons[i][j], Y_train[i][j], diff, sum_squared_diff);
                    sum_squared_diff += diff * diff;
                }
            }
            double mse = sum_squared_diff / num_train;
            ...
            // Free memory for a_temp
            for (int layer = 0; layer <= num_hidden_layers; layer++) {
                int num_neurons_current_layer = (layer == num_hidden_layers) ? num_outputs : num_neurons[layer];
                for (int neuron = 0; neuron < num_neurons_current_layer; neuron++) {
                    free(a_temp[layer][neuron]);
                }
                free(a_temp[layer]);
            }
            free(a_temp);
