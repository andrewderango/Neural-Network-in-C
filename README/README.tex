...
\item \codebox{main.c}: Defines the main function that calls functions from \codebox{mymodel.c}.
		\item \codebox{functions.c}: Defines all support functions for data handling, model training, and performance evaluation.
...
\codebox{DownloadANN} is the function responsible for downloading or saving the network that was just trained. It first asks the user if they would like to save the network, then creates a file called Downloaded Data to their current directory if they say yes and the directory does not already exist. It creates the file \codebox{ANN\_data.txt} in this folder then writes the model’s architecture, performance metrics, and weights and biases into it.
...
\subsubsection{\codebox{libsodium} Installation on WSL}
...
\begin{figure}[h]
    ...
    \caption{Beginning of \codebox{BackwardPass} function for backpropagation.}
    \label{fig:figure1}
\end{figure}
